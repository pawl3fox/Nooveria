...
import { Users, MessageSquare, Coins, Activity, TrendingUp, Database, Bot, Plus, Trash2 } from "lucide-react";
...
export function AdminInterface({ user }: AdminInterfaceProps) {
  ...
  const [newAssistant, setNewAssistant] = useState({ name: '', description: '', system_prompt: '', assistant_id: '', use_openai_assistant: false });
  ...
  useEffect(() => {
    const loadAdminData = async () => {
      try {
        ...
        // Load assistants separately with error handling
        try {
          const assistantsData = await apiService.getAssistants();
  ...
  const createAssistant = async () => {
    if (!newAssistant.name.trim() || !newAssistant.description.trim() || 
        (newAssistant.use_openai_assistant ? !newAssistant.assistant_id.trim() : !newAssistant.system_prompt.trim())) {
      return;
    }
    
    try {
      setIsCreating(true);
      const response = await fetch('http://185.163.45.160:8000/api/assistants', {
        method: 'POST',
        headers: {
        ...
        body: JSON.stringify({
          ...
          system_prompt: newAssistant.use_openai_assistant ? '' : newAssistant.system_prompt,
          assistant_id: newAssistant.use_openai_assistant ? newAssistant.assistant_id : '',
          use_openai_assistant: newAssistant.use_openai_assistant
        })
      });
      ...
      if (response.ok) {
        ...
        setNewAssistant({ name: '', description: '', system_prompt: '', assistant_id: '', use_openai_assistant: false });
      }
    } catch (error) {
  ...
  const deleteAssistant = async (assistantId: string) => {
    try {
      const response = await fetch(`http://185.163.45.160:8000/api/assistants/${assistantId}`, {
        method: 'DELETE',
        headers: {
          'Authorization': `Bearer ${localStorage.getItem('session_token')}`
        }
  ...
  if (user.role !== 'admin') {
    return (
      <div className="flex-1 p-8 bg-site min-h-full">
  ...
  return (
    <div className="absolute inset-0 bg-site overflow-y-auto">
        ...
        <Tabs defaultValue="activity" className="w-full flex flex-col">
          ...
          <TabsContent value="activity" className="mt-6">
            <div className="border border-accent rounded-lg" style={{ backgroundColor: '#1A1A1D', height: '600px' }}>
              ...
              <div style={{ height: '540px', overflowY: 'scroll', padding: '16px' }}>
                {transactions.map((tx) => (
          ...
          <TabsContent value="users" className="mt-6">
            <div className="border border-accent rounded-lg" style={{ backgroundColor: '#1A1A1D', height: '600px' }}>
              ...
              <div style={{ height: '440px', overflowY: 'scroll', padding: '16px' }}>
                {users.map((user) => (
              ...
              <Card className="border-accent backdrop-blur-sm" style={{ backgroundColor: '#1A1A1D' }}>
                ...
                <CardContent className="space-y-4">
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                    <div>
                      ...
                      <Input
                        ...
                        placeholder="Например: Помощник по выбору фильмов"
                  ...
                  <div className="flex items-center space-x-2 mb-4">
                    <input
                      type="checkbox"
                      id="useOpenAIAssistant"
                      checked={newAssistant.use_openai_assistant}
                      onChange={(e) => setNewAssistant(prev => ({ ...prev, use_openai_assistant: e.target.checked }))}
                      className="rounded border-accent"
                    />
                    <label htmlFor="useOpenAIAssistant" className="text-sm font-medium text-primary">
                      Использовать OpenAI Assistant API
                    </label>
                  </div>

                  {newAssistant.use_openai_assistant ? (
                    <div>
                      <label className="text-sm font-medium text-primary mb-2 block">OpenAI Assistant ID</label>
                      <Input
                        value={newAssistant.assistant_id}
                        onChange={(e) => setNewAssistant(prev => ({ ...prev, assistant_id: e.target.value }))}
                        placeholder="asst_xxxxxxxxxxxxxxxxx"
                        className="bg-input border-accent text-white"
                      />
                      <p className="text-xs text-secondary mt-1">ID ассистента из OpenAI Platform</p>
                    </div>
                  ) : (
                    <div>
                      <label className="text-sm font-medium text-primary mb-2 block">Системный промпт</label>
                      <Textarea
                        value={newAssistant.system_prompt}
                        onChange={(e) => setNewAssistant(prev => ({ ...prev, system_prompt: e.target.value }))}
                        placeholder="Ты - эксперт по выбору фильмов. Помогай пользователям найти идеальные фильмы..."
                        rows={4}
                  ...
                  <Button 
                    ...
                    disabled={isCreating || !newAssistant.name.trim() || !newAssistant.description.trim() || 
                      (newAssistant.use_openai_assistant ? !newAssistant.assistant_id.trim() : !newAssistant.system_prompt.trim())}
                    className="btn-gradient"
                  >
                    {isCreating ? 'Создание...' : 'Создать помощника'}
                  </Button>
                </CardContent>
              </Card>
              ...
              <Card className="border-accent backdrop-blur-sm" style={{ backgroundColor: '#1A1A1D' }}>
                <CardHeader>
                  <CardTitle className="text-primary-heading flex items-center gap-2">
                    ...
                    Активные помощники ({assistants.length})
                  </CardTitle>
                </CardHeader>
                <CardContent>
                  <div className="space-y-4">
                    {assistants.map((assistant) => (
                      <div key={assistant.id} className="p-4 border border-accent/30 rounded-lg">
                        <div className="flex items-start justify-between">
                          <div className="flex-1">
                            <h3 className="font-medium text-primary-heading mb-2">{assistant.name}</h3>
                            <p className="text-sm text-primary mb-3">{assistant.description}</p>
                            <div className="bg-accent/10 p-3 rounded border border-accent/20">
                              <p className="text-xs text-secondary mb-1">Системный промпт:</p>
                              <p className="text-sm text-primary font-mono">{assistant.system_prompt}</p>
                            </div>
                          </div>
                          <Button
                    ...
                    {assistants.length === 0 && (
                      <div className="text-center py-8">
                        <Bot className="w-12 h-12 mx-auto mb-4 text-primary-heading opacity-50" />
                        <p className="text-primary">Нет созданных помощников</p>
                        <p className="text-sm text-secondary">Создайте первого помощника выше</p>
                      </div>
                    )}
                  </div>
                </CardContent>
